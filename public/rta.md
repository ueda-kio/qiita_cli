...
- **クリップボードヒストリー機能**
  - [Clipy](https://github.com/Clipy/Clipy) ユーザーは乗り換え推奨。検索機能とサムネイル機能がすばらしい。**まごうことなき神機能**。
- **スニペット機能**
  - たまに反応してくれない時があるが **神機能**。
- **Quicklinks機能**
...
- **その他数多のextensions**
  - 先人たちのすばらしい拡張機能をインストール可能。**できないことなどない**。


### 2.2. 必須の設定

設定画面の「Advanced > Auto-switch input Source」から「Romaji（またはEnglish）」を選択してください。これによりMacの入力ソースが日本語の状態であってもRaycastを開くとアルファベット状態で開かれます。基本的にRaycastは英字入力する機能がほとんどですので、これは必須の設定です。
...
これによりもうブラウザのブクマ一覧から一生懸命探す必要も綺麗にフォルダー分けする必要もないです。部分一致でサジェストしてくれるので登録名を忘れてても見つけられます。
...
▼ サムネイル機能。とくにコピーした画像を表示してくれるのが素晴らし過ぎる。
